import os
import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from gentis_ai import Expert, Router, Flow
from gentis_ai.llm import VLLMLLM

# 1. Setup LLM
# Ensure you have a VLLM server running (e.g., python -m vllm.entrypoints.openai.api_server ...)
# Or any OpenAI compatible server.
# Example: python -m vllm.entrypoints.openai.api_server --model facebook/opt-125m

llm = VLLMLLM(
    model_name="facebook/opt-125m", # Replace with your served model name
    base_url="https://unselfish-jorden-undenotatively.ngrok-free.dev/v1",
    api_key="EMPTY"
)

# 2. Define Experts
support_expert = Expert(
    name="support",
    description="Handles technical support queries.",
    system_prompt="You are a technical support specialist. Help users with their issues."
)

sales_expert = Expert(
    name="sales",
    description="Handles sales inquiries and pricing.",
    system_prompt="You are a sales representative. Answer questions about pricing and features."
)

# 3. Setup Router
router = Router(
    experts=[support_expert, sales_expert],
    llm=llm
)

# 4. Create Flow
flow = Flow(router=router, llm=llm) 

# 5. Run Conversation
print("--- Turn 1 ---")
try:
    response = flow.process_turn("I have a problem with my account.")
    print(f"Agent: {response.agent_name}")
    print(f"Response: {response.content}")
    print(f"Token Usage: {response.token_usage}")

    print("\n--- Turn 2 ---")
    response = flow.process_turn("How much does the premium plan cost?")
    print(f"Agent: {response.agent_name}")
    print(f"Response: {response.content}")
    print(f"Token Usage: {response.token_usage}")
except Exception as e:
    print(f"Error: {e}")
    print("Make sure a VLLM/OpenAI compatible server is running at http://localhost:8000/v1")
